...
- [x] **5.1 `buildReport`**: блоки из [feedback.md](feedback.md), тесты на
  «лучше было бы», темы, расхождение версий.
- [x] **5.2 Рекомендации**: слабые темы, кандидаты из каталога, тесты на
  сортировку и лимит.
- [x] **5.3 Экран отчёта**: `containers/scenario-report/`, страница,
  действия. Docs: features/scenario-run.md.

## Этап 6 — Курсы и результаты

- [x] **6.1 Лучшая попытка**: `bestAttempt`, `scenarioStatus`,
  `courseProgress` по правилу сравнения в `utils/scenario-engine/`, тесты.
- [x] **6.2 Страницы курсов**: `ROUTES.COURSES`, `ROUTES.COURSE`, список
  курсов, курс со сценариями и статусом каждого, «К курсу» в отчёте и
  выход из сценария на курс («Следующий сценарий» — с шага 5.3). Навигация в
  `AppLayout`, ссылка с главной.
  Docs: routing.md, features, product/flows (курс).
- [x] **6.3 История попыток**: список попыток сценария в курсе с
  переходом в отчёт.
