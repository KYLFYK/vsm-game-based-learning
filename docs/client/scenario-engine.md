...
`crypto.randomUUID()` браузер отдаёт только в безопасном контексте
(HTTPS или `localhost`). Dev-сервер слушает `server.host: true`
([vite.config.ts](../../vite.config.ts)), то есть доступен по IP из
локальной сети; открыв его по обычному HTTP с другого устройства,
получите исключение при старте сценария — `runStarted` упадёт на
вызове `crypto.randomUUID()`. Продакшен всегда за TLS, там ограничение
не действует. Обхода в проекте нет: с того же компьютера открывайте
`localhost`, с другого устройства — только через HTTPS (в репозитории
готового `--https`-флага или туннеля нет, нужно поднять самостоятельно).
