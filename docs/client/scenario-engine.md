...
- `optionChosen(optionId)` — выбор варианта узла `choice`: применяет
  ...
  по `next`.
- `expired()` — диспатчит хук таймеров (этап 3.2), когда дедлайн истёк
  вне клика игрока; завершает попытку `timeout`, если дедлайн правда
...
`useRunTimers` в контейнере сцены (этап 3.2): собственный `now` с
интервалом около 250 мс, диспатч `expired()`, когда остаток ушёл в
ноль. Это единственное место, где время живёт вне store, и оно
производное от дедлайнов. Таймер узла заводится при входе в узел и
снимается при выходе; возврат в тот же узел заводит его заново. Паузы
нет.
