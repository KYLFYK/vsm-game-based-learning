...
| `SpeechBubble` `Left`/`Right` | Белое облако, скругление 22 px, острый угол со стороны персонажа, именная плашка синего цвета на этом углу с наклоном; тень `redDrop` (`filter: drop-shadow`), наклон `-tilts.sm` |
...
- **Финал**: сцена без вариантов; «К отчёту» (`Primary`, `Lg`) снизу по
  центру, `Stamp` по центру над кнопкой (`stage.stampBottom`), чтобы не
  наезжать на подпись автора сверху. Ошибка сохранения — под кнопкой,
  вне потока: кнопка не сдвигается к штампу.
