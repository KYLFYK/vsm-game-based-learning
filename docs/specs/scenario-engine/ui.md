...
- `ScenarioPage` при размонтировании диспатчит `runLeft`. Перезагрузка
  открывает заставку заново: оборванная попытка не оживает.
...
`speaker-layout.ts`, `use-run-timers.ts`, `use-player-keys.ts`,
`use-fullscreen.ts`, `scenario-player.styles.ts`.
...
- «Выйти»: `window.confirm('Попытка не сохранится. Выйти?')` → `runLeft`
  и переход на курс (если есть `course`) или `HOME`. На финале вопрос
  тот же: попытка сохраняется только кнопкой «К отчёту».
...
- Слоты: 30 % ширины зоны, прижаты к низу; облако со стороны центра,
  ширина до 40 %.
